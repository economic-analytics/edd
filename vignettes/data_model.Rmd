...
Dates are always stored as Date objects and returned in ISO8601 format
...
annual series), there is a separate variable, `freq`, that holds a
...
`dates <- data.frame(date = c("2022-07-01", "2022-10-01"), freq = rep("q", 2)) date_to_text(dates)`
...
occupies significant amounts of memory where datasets are particulalry
...
`variable variable.code | variable.name | variable.unit`
...
`dates | geography | industry | variable | date | freq | code | name | code | name | code | name | unit | value`
...
`master <- data.frame(dates = data.frame(date, freq),                     geography = data.frame(code, name),                     industry = data.frame(code, name),                     variable = data.frame(code, name, unit),                     value                     )`
...
dimensions are easily passed around between functions be simply
...
dataset \<- list( data = tibble( dates = tibble( date, freq), industry,
geography, variable, value), dimensions = list( geography = tibble(
code, name), industry = tibble( code, name), variable = tibble( code,
name, unit) ) )
...
verification functions (`as_eddobj()`, `is_eddobj()`) are available, as

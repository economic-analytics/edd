ons_process_rgva <- function(filepath = NULL, build_for_fedo = TRUE) {
  ...
  rgva <- lapply(rgva_sheets, function(sht) readxl::read_excel(path = filepath,
                                                               ...
                                                               skip = 1,
                                                               na = "c"))
  names(rgva) <- c("ITL1_cvm", "ITL1_constant", "ITL1_current", "ITL1_deflators",
  ...
  rgva <- rgva %>%
    ...
    dplyr::mutate(dates = stringr::str_sub(dates, 1, 4),
                  ...
                  industry_code = dplyr::case_when(grepl("[0-9]", industry_code) &
                  ...
                  geography_type = dplyr::case_when(geography_code == "UK"  ~ "ctry",
                                               ...
                                               TRUE ~ geography_type)) %>%
    dplyr::select(dates, geography_type, geography_code, geography_name, industry_code, industry_name, variable, value)
...
ons_rgva_to_fedo <- function(rgva) { # takes the processed rgva file
  ...
  readr::write_rds(rgva_final, "data/RGVA.rds")

edd_obj_to_dataframe <- function(edd_obj) {
  # identify all dimensions in the edd_obj that's been passed in
  ...
  for (d in seq_along(dims)) {
    ...
    data <- dplyr::left_join(
      data,
      dimension |>
        dplyr::select(name, code), # the select restriction is in place until we can dynamically handle $unit, $type, etc.
